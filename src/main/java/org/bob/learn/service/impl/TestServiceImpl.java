...
public class TestServiceImpl implements TestService {
    ...
    public void print(String mac) {
        RedisUtils.get(String.format(RedisKey.GATEWAY_PROVINCE.getPattern(),mac,null)).subscribe(log::info);
        counter.increment();
        if(counter.longValue()%5==0){
            RedisUtils.set(String.format(RedisKey.GATEWAY_BIND.getPattern(),mac,null),"1",604800).subscribe();
        }
    }
}
